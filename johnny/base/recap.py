...
def get_chains_at_date(
    ...
    commfees = transactions.select(lambda r: r.datetime.date() == date).aggregate(
        "chain_id", {"commissions": ("commissions", sum), "fees": ("fees", sum)}
    )

    # Process the chains.
    chains = (
        chains.select(lambda r: r.mindate <= date <= r.maxdate)
