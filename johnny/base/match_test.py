...
class TestMatch2(unittest.TestCase):
    ...
    @mock.patch.object(match, '_GetMarkTime',
                       return_value=datetime.datetime(2021, 7, 1, 0, 0, 0))
    def test_add_missing_expirations(self, _):
        header = ('account', 'datetime', 'transaction_id', 'order_id', 'rowtype', 'symbol', 'instruction', 'effect', 'quantity', 'cost', 'description', 'price', 'commissions', 'fees')
        transactions = petl.wrap([
            header,
            ('A', datetime.datetime(2021, 6, 1, 12, 10, 0), '00000001', None, 'Trade', 'AAPL_210625_C150', 'BUY', 'OPENING', Decimal('2'), Decimal('123.00'), 'Desc', ZERO, ZERO, ZERO),
        ])
        expected_output = petl.wrap([
            header,
            ('A', datetime.datetime(2021, 6, 1, 12, 10, 0), '00000001', None, 'Trade', 'AAPL_210625_C150', 'BUY', 'OPENING', Decimal('2'), Decimal('123.00'), 'Desc', ZERO, ZERO, ZERO, '&00000001'),
            ('A', datetime.datetime(2021, 6, 26, 0, 0, 0), '62535baf0310', 'e60a324c', 'Expire', 'AAPL_210625_C150', 'SELL', 'CLOSING', Decimal('2'), Decimal(0), 'Synthetic expiration for AAPL_210625_C150', ZERO, ZERO, ZERO, '&00000001'),
        ])
    ...
    def test_add_mark_transactions(self, _):
        ...
        transactions = petl.wrap([
            header,
            ('A', datetime.datetime(2021, 7, 1, 12, 10, 0), '00000001', None, 'Trade', 'AAPL_210925_C150', 'BUY', 'OPENING', Decimal('2'), Decimal('123.00'), 'Desc', ZERO, ZERO, ZERO),
        ])
        expected_output = petl.wrap([
            header,
